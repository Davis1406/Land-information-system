...
{
$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.location','s.contact','s.joindate');
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "s.id";
	
	/* DB table to use */
	$sTable = " clients as s,land as b ";
	...
	{
	$condArr[] = "s.sname like '%".mysqli_real_escape_string($conn,$_GET['clients'])."%'";
	
	}
	
	if(isset($_GET['land']) && $_GET['land']!="")
	{
	$condArr[] = "b.id = '".mysqli_real_escape_string($conn,$_GET['land'])."'";
	...
	$sWhere = " WHERE b.id=s.location and s.delete_status='0' and s.balance>0 ";
	...
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as cc
		FROM   ".$sTable." WHERE b.id=s.location and s.delete_status='0' and s.balance>0  ";
	...
	while ( $aRow = $rResult->fetch_assoc()  )
	{
		...
		$row = array(
					...
					html_entity_decode('<button class="btn btn-warning btn-xs" onclick="javascript:GetFeeForm('.$aRow['id'].')"><i class="fa fa-inr "></i> View Details </button>')
...
$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.location','s.contact','s.joindate');
	...
	$sWhere = " WHERE b.id=s.location and s.delete_status='0'  ";
	if ( isset($_GET['sSearch'])&& $_GET['sSearch'] != "" )
	{
	...
	/* Total data set length */
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as cc
		FROM   ".$sTable." WHERE b.id=s.location and s.delete_status='0'   ";
	$rResultTotal = $conn->query( $sQuery ) or die(mysqli_error($conn));
	$aResultTotal = $rResultTotal->fetch_assoc();
	$iTotal = $aResultTotal['cc'];
	...
	while ( $aRow = $rResult->fetch_assoc()  )
	{
		...
		$row = array(
					...
					html_entity_decode('<button class="btn btn-warning btn-xs" onclick="javascript:GetFeeForm('.$aRow['id'].')"><i class="fa fa-inr "></i>View Report</button>')				
                );
