...
$location = mysqli_real_escape_string($conn,$_POST['location']);
  ...
  $q1 = $conn->query("INSERT INTO clients (sname,joindate,contact,about,emailid,location,balance,fees,dot,land_terms,office_number) 
  VALUES ('$sname','$joindate','$contact','$about','$emailid','$location','$balance','$fees','$dot','$land_terms','$office_number')") ;
  
  $sid = $conn->insert_id;
  
 $conn->query("INSERT INTO  payments (stdid,paid,submitdate,transcation_remark) VALUES ('$sid','$advancefees','$joindate','$remark')") ;
 ...
 $id = mysqli_real_escape_string($conn,$_POST['id']);	
 $sql = $conn->query("UPDATE  clients  SET  sname ='$sname',joindate='$joindate',contact='$contact',balance='$balance',fees='$fees',dot='$dot',land_terms='$land_terms',office_number='$office_number' WHERE  id  = '$id'");
 echo '<script type="text/javascript">window.location="clients.php?act=2";</script>';
 }	 
...
$conn->query("UPDATE  clients set delete_status = '1'  WHERE id='".$_GET['id']."'");	
...
$sqlEdit = $conn->query("SELECT * FROM clients WHERE id='".$id."'");
if($sqlEdit->num_rows)
{
			...
			<script type="text/javascript" src="js/validation/jquery.validate.min.js"></script>
               ...
               <div class="panel panel-primary">
						...
						<div class="form-group">
								...
								<div class="col-sm-10">
									<input type="text" class="form-control" id="contact" name="contact" value="<?php echo $contact;?>" maxlength="14" />
								</div>
							</div>
						...
						<div class="form-group">
								...
								<div class="col-sm-10">
                                    ...
                                    <?php
									$sql = "select * from land where delete_status='0' order by land.location asc";
									$q = $conn->query($sql);
									
									while($r = $q->fetch_assoc())
									{
									echo '<option value="'.$r['id'].'"  '.(($location==$r['id'])?'selected="selected"':'').'>'.$r['location'].'</option>';
									}
									?>									
                        ...
                        <div class="panel-body">
                            <div class="table-sorting table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="tSortable22">
                                    ...
                                    <tbody>
									<?php
									$sql = "select * from clients where delete_status='0'";
									$q = $conn->query($sql);
									$i=1;
									while($r = $q->fetch_assoc())
									{
